@props(['color' => '#ffffff', 'width' => '32', 'height' => '32', 'type' => 'default'])

@switch($type)
    @case('brand')
        <svg id="code_snippet" data-name="code snippet" height="{{ $height }}" viewBox="0 0 104 24"
            width="{{ $width * 4 }}" xmlns="http://www.w3.org/2000/svg">

            <style>
                .brand {
                    font: bold {{ $height }} monospace;
                    fill: {{ $color }};
                }
            </style>
            ...
            <text class="brand" x="26" y="18">Snippetly</text>
        </svg>
    @break

    @default
        <svg id="code_snippet" data-name="code snippet" height="{{ $height }}" viewBox="0 0 24 24"
